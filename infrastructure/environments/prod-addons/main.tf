...
provider "kubernetes" {
  host = data.terraform_remote_state.prod.outputs.cluster_endpoint

  token = data.aws_eks_cluster_auth.this.token

  cluster_ca_certificate = base64decode(
    data.terraform_remote_state.prod.outputs.cluster_ca
  )
}
...
module "addons" {
  ...
  cluster_ca        = data.terraform_remote_state.prod.outputs.cluster_ca

  oidc_provider_arn = data.terraform_remote_state.prod.outputs.oidc_provider_arn
